mutable struct SpacecraftElement
    ID::String
...
function SpacecraftElement(;
    ...
    return SpacecraftElement(ID, geometry, mass, inertiaE_E, inertiaG_E, R_OE, posEG_E,
        posOE_O, posOG_O, inertiaO_O, inertiaG_O, LO_O, LG_O, freq, damp, isFlex)
end

function Base.show(io::IO, obj::SpacecraftElement)
    println(io, "ID:         $(obj.ID)")
    println(io, "type:       $(typeof(obj))")
    println(io, "mass:       $(round(obj.mass, digits=2)) kg")
    println(io, "posOG_O:    $(round.(obj.posOG_O, digits=3)) m")
    println(io, "inertia at CoM:")
    println(io, "  Ixx, Iyy, Izz: $(round.(diag(obj.inertiaG_O), digits=1)) kg m²")
    println(io, "  Ixy, Ixz, Iyz: $(round.([obj.inertiaG_O[1, 2], obj.inertiaG_O[1, 3], obj.inertiaG_O[2, 3]], digits=1)) kg m²")
    #if obj.isFlex
    #    println(io, "freq:          $(round.(obj.freq, digits=2)) rad/s")
    #    println(io, "damp:          $(round.(obj.damp*100, digits=2)) %")
    #    println(io, "LO_O:       $(round.(obj.LO_O, digits=2))")
    #end
end
